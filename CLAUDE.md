...
**The `to_do` status group is one list.** `create-task.js` lands a new task on `Not started`; the board and the picker must both consider it offerable, and for a while they did not — `get-board.js` filtered its default listing on the single option `"To do"`, so a task created with defaults was invisible in the bare `get-board.js` (spovishun-193). Membership now lives once, in `TODO_GROUP_STATUSES` (`hooks/notion-constants.js`), **in preference order**. The two consumers read it differently on purpose: `get-board.js` ORs the whole list into one filter (via `statusClause` in `scripts/notion/lib/query-tasks.js`, which also lets `queryByPriorityTier` take a list), while `hooks/task-picker.js` destructures it into a primary and a fallback phase — only the fallback emits `--from-not-started`, the flag `apply-pick.js` needs to promote `Not started → To do`, so collapsing the picker's two queries into one OR would lose the promotion. Do not re-add a creation-side default that skips grooming: `DEFAULT_STATUS` staying `Not started` is the decision (1.8.0, #107), and `test/notion-status-defaults.test.js` evaluates the board's default filter against the Status `create-task.js` actually writes.
