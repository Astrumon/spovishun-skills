...
**Block tree is fetched once, rendered twice.** `hooks/block-tree.js` (re-exported by `scripts/notion/lib/block-tree.js`) owns the only walk that paginates a block level and recurses into `has_children`, attaching results as `block.children`. It takes the caller's page-fetch as a **callback**, not an HTTP module — that is what lets both trees share it while their transports stay deliberately different. Depth is a hard budget (`MAX_BLOCK_DEPTH = 3`): at 0 a block's children are left unfetched rather than half-attached. Every renderer downstream (`format-task.js`, `hooks/notion-blocks.js`) is **pure and synchronous** over an already-hydrated tree; do not move a fetch into one. Hydration is not optional decoration: `newtask` puts the agent prompt inside a toggle and `notion-task-to-code` reads it back, so a renderer that ignores children hands the agent a task with no prompt (spovishun-185). The two renderers still differ on purpose — `format-task.js` separates standalone blocks so its output round-trips through `markdown-to-blocks.js`, asserted by `test/scripts-notion-format-task.test.js` — but that gap is now small enough that unifying them behind one renderer with options is a live follow-up, not a rejected idea.
