...
// Note: `rules/` is intentionally excluded. Rule files are flat .md data
// (no per-artifact manifest), copied/inlined directly by each adapter.

const TEXT_EXTENSIONS = new Set(['.md', '.txt', '.json', '.yaml', '.yml', '.html']);

/**
 * Loads all artifacts from the given package root directory.
 * Each artifact directory must contain manifest.yaml and SKILL.md / AGENT.md / TEMPLATE.md / (hook executable).
 *
...
function loadArtifact(artifactDir, kind, dirName) {
  ...
  const bodyFilename = resolveBodyFilename(artifactDir, kind);
...
function resolveBodyFilename(artifactDir, kind) {
  const filename = BODY_FILES[kind];
  if (filename !== null) return filename;
  // hooks: pick the first non-manifest, non-hidden top-level file in sorted
  // order — sorting keeps the choice deterministic across filesystems.
  const files = readdirSync(artifactDir)
    .filter((f) => f !== 'manifest.yaml' && !f.startsWith('.') && statSync(join(artifactDir, f)).isFile())
    .sort();
  return files[0] ?? null;
}

function readBodyText(artifactDir, kind) {
  const filename = resolveBodyFilename(artifactDir, kind);
  if (filename === null) {
    throw new ConfigError(
      'MISSING_REQUIRED',
      `${kind} directory ${artifactDir} has no body file besides manifest.yaml.`,
      `Add the ${kind} body file to the directory.`
    );
  }
