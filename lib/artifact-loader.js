...
function resolveBodyFilename(artifactDir, kind) {
  ...
  // hooks: pick whichever non-manifest, non-hidden file exists at the top level.
  const files = readdirSync(artifactDir).filter(
    (f) => f !== 'manifest.yaml' && !f.startsWith('.') && statSync(join(artifactDir, f)).isFile()
  );
  return files[0] ?? null;
}
...
function readBodyText(artifactDir, kind) {
  const filename = BODY_FILES[kind];
  if (filename === null) {
    // hooks: read whichever non-manifest, non-hidden top-level file exists
    const files = readdirSync(artifactDir).filter(
      (f) => f !== 'manifest.yaml' && !f.startsWith('.') && statSync(join(artifactDir, f)).isFile()
    );
    if (files.length === 0) {
