 ...
 * @returns {Map<string, {repo: string, ref?: string, pins: Set<string>}>}
  ...
  for (const artifactDir of ARTIFACT_DIRS) {
    ...
    for (const name of readdirSync(baseDir)) {
      ...
      out.set(`${artifactDir}/${name}`, {
        repo: source.repo,
        ref: source.ref,
        pins: new Set((source.files ?? []).map((f) => pin(f.path, f.sha))),
      });
    }
  }
 ...
 *
 * @param {Map<string, {pins: Set<string>}>} sourcePins  from `collectSourcePins`
...
export function diffPins(sourcePins, noticePins) {
  ...
  for (const [artifact, { pins }] of sourcePins) {
