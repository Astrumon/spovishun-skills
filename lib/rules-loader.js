 ...
 *
 * @param {{id: string, body: string}} rule — as returned by collectRules
 * @param {object} configMap — placeholder map from buildPlaceholderMap(config)
 * @returns {{kind: 'rule', id: string, version: string, checksum: string}}
 */
export function ruleLockEntry(rule, configMap) {
  return {
    kind: 'rule',
    id: rule.id,
    version: RULE_LOCK_VERSION,
    checksum: sha256(renderRule(rule, configMap)),
  };
}
