 ...
 *
 *   'checksum' — rules. They have no YAML frontmatter, so there is no marker to
 *               consult and ownership is decided by content alone: a body equal to
 *               what we would write is ours (UNCHANGED, short-circuited below),
 *               and anything else is ours only if the lockfile already tracks the
 *               id. A locked rule that drifted is therefore LOCAL_ONLY/CONFLICT —
 *               reported and kept — never DISOWNED, which would drop the entry.
 *
 *   'assume-owned' — every file is ours. Used by targets whose ownership model is
 *               still open (windsurf chunked -part-N files; see spovishun-162).
 *
...
function isOwned({ ownership, lockEntry, onDiskChecksum, hasMarker }) {
  ...
  if (ownership === 'assume-owned' || ownership === 'none') return true;
 ...
 * @param {'marker'|'checksum'|'assume-owned'} [opts.ownership] — ownership model (default 'marker')
